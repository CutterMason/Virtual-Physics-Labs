...
public class Position2Graph : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    void Update()
    {
        ...
        if (timer >= updateInterval)
        {
            ...
            float relativePosition = GetRelativePosition();
            AddValue(relativePosition);
            RedrawGraph();

            if (yAxisLabel) yAxisLabel.text = $"Pos: {relativePosition:F2}";
