...
public class Velocity2Graph : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    void Awake()
    {
        ...
        // Center containers and match panel size
    ...
    void Update()
    {
        ...
        {
            timer = 0f;
            float v = GetRelativeVelocity();
            AddValue(v);
            RedrawGraph();

            if (yAxisLabel) yAxisLabel.text = $"Vel: {v:F2}";
    ...
    void DrawSegment(RectTransform container, Vector2 start, Vector2 end, Color color)
    {
        ...
        rt.anchorMin = rt.anchorMax = new Vector2(0.5f, 0.5f); // center anchor
        rt.pivot = new Vector2(0, 0.5f); // start horizontally
    ...
    void DrawGrid(float width, float height)
    {
        ...
        int needed = gridLinesX + gridLinesY + 1; // +1 for zero line
